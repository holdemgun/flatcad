...
public abstract class Builtin {
	...
	public static Map<String, Builtin> getBuiltins(
			final FlatInterpreter interpreter) {
		...
		builtins.put("__split", new Builtin() {
			public Register eval(List<Register> args) {
				...
				if (args.size() == 3) {
					ret = splitList(args.get(0), (Register.List) args.get(1),
							args.get(2));
				}
				return ret;
			}
		...
		builtins.put("__splitAtIndex", new Builtin() {
			public Register eval(List<Register> args) {
				Register ret = Register.getNil();
				if (args.size() == 2) {
					ret = splitListAtIndex(args.get(0),
							(Register.List) args.get(1));
				}
				return ret;
			}
		});
			...
			public Register eval(List<Register> args) {
				...
				try {
					if (r instanceof Register.List) {
						String src = interpreter
								.unlist((Register.List) r, true);
						ret = new Register.Str(src);
					} else if (r instanceof Register.Function) {
						Register.Function f = (Register.Function) r;
						Register.List intermediate = interpreter.quoteChildren(
								f.block, true, null);
						...
						paramList.add(")");
						Register.List params = interpreter
								.listFromList(paramList);
		...
		builtins.put("__replaceNode", new Builtin() {
			public Register eval(List<Register> args) {
				if (args.size() == 3 && args.get(2) instanceof Register.List) {
					return modifyList(args.get(0), args.get(1),
							(Register.List) args.get(2), false, false);
				} else {
		...
		builtins.put("__insertNode", new Builtin() {
			public Register eval(List<Register> args) {
				if (args.size() == 3 && args.get(2) instanceof Register.List) {
					return modifyList(args.get(0), args.get(1),
							(Register.List) args.get(2), true, false);
				} else {
					Debug.out("FlatInterpreter", "__replace requires a,b, list");
					return Register.getNil();
				}
		...
		builtins.put("__replaceList", new Builtin() {
			public Register eval(List<Register> args) {
				if (args.size() == 3 && args.get(2) instanceof Register.List) {
					return modifyList(args.get(0), args.get(1),
							(Register.List) args.get(2), false, true);
				} else {
					Debug.out("FlatInterpreter", "__replace requires a,b, list");
					return Register.getNil();
				}
		...
		builtins.put("__insertList", new Builtin() {
			public Register eval(List<Register> args) {
				if (args.size() == 3 && args.get(2) instanceof Register.List) {
					return modifyList(args.get(0), args.get(1),
							(Register.List) args.get(2), true, true);
				} else {
					Debug.out("FlatInterpreter", "__replace requires a,b, list");
					return Register.getNil();
				}
		...
		builtins.put("__cons", new Builtin() {
			public Register eval(List<Register> args) {
				...
				// } else {
				// Debug.out("FlatInterpreter",
				// "__cons: 2nd arg must be a list");
				// return Register.getNil();
				// }
		...
		builtins.put("__first", new Builtin() {
			public Register eval(List<Register> args) {
				...
				if (args.get(0) instanceof Register.List) {
					...
					if (r.root == null) {
						Debug.out("FlatInterpreter",
								"__first: list empty, I return nil");
					} else {
						ret = r.root;
					}
				} else {
					Debug.out("FlatInterpreter",
							"__first: argument not a list, returning it");
					return args.get(0);
				}
				return ret;
			}
		...
		builtins.put("__part", new Builtin() {
			public Register eval(List<Register> args) {
				if (args.size() == 1) {
					System.out.println("Beginning a new part named "
							+ args.get(0).toString());
					catchTurtle(Turtle.part(args.get(0).toString()),
							interpreter);
				} else {
		...
		builtins.put("__forward", new Builtin() {
			public Register eval(List<Register> args) {
				catchTurtle(Turtle.forward(MathUtils.toFloat(args.get(0)
		...
		builtins.put("__backward", new Builtin() {
			public Register eval(List<Register> args) {
				catchTurtle(
						Turtle.forward(-1f
								* MathUtils.toFloat(args.get(0).toString())),
						interpreter);
				return Register.getNil();
			}
		});
		...
		builtins.put("__pitch", new Builtin() {
			public Register eval(List<Register> args) {
				catchTurtle(
						Turtle.pitch(MathUtils.toFloat(args.get(0).toString())),
						interpreter);
				return Register.getNil();
			}
		});
		...
		builtins.put("__mark", new Builtin() {
			public Register eval(List<Register> args) {
				catchTurtle(Turtle.mark(args.get(0).toString()), interpreter);
				return Register.getNil();
			}
		});
		...
		builtins.put("drawto", new Builtin() {
			public Register eval(List<Register> args) {
				catchTurtle(drawto(interpreter, args), interpreter);
				return Register.getNil();
			}
		});
		...
		builtins.put("outputMatrix", new Builtin() {
			public Register eval(List<Register> args) {
				MathUtils.debugMatrixString("current stack at " + args.get(0),
						getCurrentTransform(interpreter));
				return Register.getNil();
			}
		});
		...
		builtins.put("facedir", new Builtin() {
			public Register eval(List<Register> args) {
				catchTurtle(facedir(interpreter, (Register.Mat) args.get(0)),
						interpreter);
				return Register.getNil();
			}
		});
		...
		builtins.put("__showPolygons", new Builtin() {
			public Register eval(List<Register> args) {
				boolean v = args.size() > 0
						&& args.get(0).toString().equals("true");
		...
		builtins.put("__hpgl", new Builtin() {
			public Register eval(List<Register> args) {
				try {
					...
					Debug.out("FlatInterpreter",
							"Some kind of weird exception in hpgl, as follows...");
					ex.printStackTrace();
				}
				return Register.getNil();
			}
		...
		builtins.put("laser", new Builtin() {
			public Register eval(List<Register> args) {
				...
				Debug.out("Builtin", "did laser mode, saved to "
						+ args.get(0).toString());
				return Register.getNil();
			}
		});
		...
		builtins.put("sketchup", new Builtin() {
			public Register eval(List<Register> args) {
				...
				Debug.out("Builtin",
						"did sketchup mode, saved to " + args.get(0).toString());
				return Register.getNil();
			}
		});
	...
	/**
	 * Join all of the objects in the input list into a single return list. This
	...
	/**
	 * Given a target object and a list, split the list into two sub-lists and
	 * return them in a two-element list. The searched object is the first
	 * element in the second list. If the target object is not found, this still
	 * returns a tuple, but the second one is empty.
	 * 
	 * splitList("c", ["a", "b", "c", "d"]) ==> [["a", "b"], ["c", "d"]
	 * 
	 * splitList("X", ["a", "b", "c", "d"]) ==> [["a", "b", "c", "d"], []]
	 */
	...
	public boolean prettyPrint(Register r, StringBuffer buf, int level,
			boolean justFinishedList) {
		boolean ret = false;
		if (buf.length() > 0) {
			buf.append(' ');
		}
	...
	public Register.List modifyList(Register lookFor, Register replaceWith,
			Register.List list, boolean insertMode, boolean unpackReplaceWith) {
		Register uberNext;
	...
	public void doBackto(String where, FlatInterpreter interpreter) {
		TurtleTree turtles = interpreter.soup.getSourceList(); // get whatever
																// list we're
																// working with
																// atm
	...
	public void drawShape(String shapeName, String optionalStartPoint,
		...
		if (optionalStartPoint == null) {
			...
			Turtle startPoint = shapeTurtles.getRoot().search(
					optionalStartPoint);
			if (startPoint != null) {
				...
				// now throw from the root (inclusive) to startPoint (not
				// inclusive)
	...
	public Register dir(FlatInterpreter interpreter) {
		...
		Register ret = new Register.Mat(u.x(), u.y(), u.z(), v.x(), v.y(),
				v.z(), w.x(), w.y(), w.z());
		return ret;
	}
	...
	public Turtle drawto(FlatInterpreter interpreter, List<Register> args) {
		...
		Vertex spanningVec = new Vertex(thereVec.x() - hereVec.x(),
				thereVec.y() - hereVec.y(), thereVec.z() - hereVec.z());

		Vertex desired = new Vertex(hereVec.x()
				+ (spanningVec.x() * percentDist), hereVec.y()
				+ (spanningVec.y() * percentDist), hereVec.z()
				+ (spanningVec.z() * percentDist));
