...
/**
 * A Polygon consisting of an ordered list of Vertex objects. It is held
 * together using HalfEdge data structures.
 * 
 * <p>
 * Instance methods give you information about this polygon, such as the list of
 * vertices, half-edges, line segments, the surface normal, and so on. It
 * includes a bunch of methods that aren't strictly necessary but make life
 * easier, such as questions 'is this point on the plane of this polygon?' (
 * <tt>isOnPlane(Vertex)</tt>), or 'what is the longest line segment connecting
 * two nonadjacent edges?' (<tt>findLongestMidline()</tt>). (Note that
 * <tt>isOnPlane</tt> is actually in the superclass, Plane, so look there for
 * more.)
 * 
 * <p>
 * The most important thing to keep in mind when using this polygon is that the
 * vertices must be supplied in <i>counter-clockwise order</i>, assuming you are
 * looking at the top of the polygon. The surface normal will then be pointing
 * up towards you. When this is drawn in OpenGL, it is likely that the polygon
 * will be invisible if you are looking at the back of the polygon.
 **/
public class Polygon extends Plane implements GLDrawable, Namable, Visitable {
	...
	/**
	 * A reference to one of the half-edges in this polygon. All others can be
	 * accessed from here.
	 */
	...
	/**
	 * The edge that was last visited when sitting on this vertex. Note that it
	 * may be null.
	 */
	protected HalfEdge lastVisited;

	/**
	 * Make a blank polygon. You probably should not use this, since it is only
	 * here because JUnit wants it.
	 */
	public Polygon() {
		super();
		// JUnit demands
	...
	/**
	 * Make a polygon with the given list of vertices. The vertices should be
	 * given in counter-clockwise direction as you are looking at the 'front' of
	 * the polygon (with the surface normal poking you in the eye).
	 */
	...
	/**
	 * This provides a 2D bounding box that only looks at the x and y values.
	 */
	...
	/**
	 * Indicates that the visited thing (such as a polygon or vertex) is just
	 * now being visted along the given half-edge.
	 */
	...
	/**
	 * Establishes all the points used in this polygon (throwing out the old
	 * ones, if any). This is essentially the init method.
	 */
	public final void usePoints(List<Vertex> points) {
		...
		// Debug.out("Polygon", "Initialized a polygon with points: " +
		// Debug.num(points, " ") + ".");
		// Debug.out("Polygon", "My edge is " + edge + ".");
		// Debug.out("Polygon", "My surface normal is: " + getNormal() + ".");
		// Debug.out("Polygon", "My internal list of points is: " +
		// getVertices() + ".");
	}

	private void setColor(Color c) {
		// Debug.out("Polygon", "changing color to " + c + " in polygon " +
		// hashCode());
		red = ((double) c.getRed()) / 255.0;
		green = ((double) c.getGreen()) / 255.0;
		blue = ((double) c.getBlue()) / 255.0;
		// Debug.out("Polygon", "colormeister gave me " + c + " r: " + red +
		// " g: " + green + " b: " + blue);
	}

	/**
	 * Returns one of the half-edges used in this polygon. You can get the
	 * others by following the edge around.
	 */
	...
	/**
	 * Sets a half-edge contained in this polygon. Be careful that you set this
	 * to a half-edge for a vertex that is actually in the polygon, or you'll
	 * have problems.
	 */
	...
	/**
	 * Returns the ordered list of line segments. It is undefined regarding
	 * which particular line segment will be first, but it is guaranteed that
	 * each successive line segment follows the previous in counter-clockwise
	 * order.
	 */
	public List<LineSegment> getSegments() {
		...
		do {
			ret.add(new LineSegment(cursor.getPoint(), cursor.getNext()
					.getPoint()));
	...
	/**
	 * Finds the half-edge that represents a line segment that contains the
	 * indicated Vertex, or null if none are found.
	 */
	...
	/**
	 * If this polygon contains an edge between the start and end vertices,
	 * return the half-edge representing that connection, null otherwise. TODO:
	 * this implementation is pretty bone-headed and should be redone sometime,
	 * but it works so I'll leave it as-is for now.
	 */
	...
	/**
	 * Returns two polygons that result from splitting this polygon along a line
	 * formed between the two vertices. For best results, make sure these
	 * vertices are actually on this polygon, and ensure that they are not
	 * adjacent.
	 * 
	 * <p>
	 * Both a and b MUST be .equals() to one of the vertices in this polygon's
	 * vertex list. You might consider looping through getVertices() in order to
	 * get an exact reference.
	 */
	public Polygon[] split(Vertex a, Vertex b) {
		...
		if (heA == null || heB == null) {
			Debug.out("Polygon", "Can't find half-edge for a or b: " + heA
					+ " or " + heB);
		} else {
	...
	/**
	 * Sets the name of this polygon, used for debugging, but could also be made
	 * to do other tricks.
	 */
	...
	/**
	 * Returns the Vertex at the end of this Polygon's half-edge. Note that this
	 * overrides Plane.getPoint() because Plane stores a vertex that is provided
	 * to it at construction time, but our our edge may change due to insertions
	 * and such.
	 */
	...
	/**
	 * Insert a Vertex into this polygon. In order for this to work, the given
	 * 'addMe' vertex MUST be along one of the edges. Otherwise it will throw an
	 * IllegalGeometryException.
	 */
	public void insert(Vertex addMe) {
		HalfEdge he = search(addMe);
		if (he == null) {
			throw new IllegalGeometryException(
					"There is no edge that can claim this point.");
		}
		he.insert(addMe);
	}

	/**
	 * Calculate the area of this polygon, assuming that each triangle involving
	 * the first point does not cross any other triangle. To be totally safe,
	 * your polygon should be convex, but that is not strictly required for this
	 * to work.
	 */
	public double area() {
		double sum = 0d;
		...
		int n = points.size() - 1;
		for (int i = 1; i < n; i++) {
			sum += MathUtils.calculateArea(points.get(0), points.get(i),
					points.get(i + 1));
		}
		return sum;
	}
	...
	/**
	 * Create a polygon that has been rotated to the XY plane and translated
	 * such that its bounding box's bottom left sits on the origin.
	 */
	public Polygon getXYPolygon(boolean translate) {
		...
		RealMatrix trn = translate ? MathUtils.getTranslationMatrix(edge
				.getPoint().getReverse()) : MathUtils
				.getTranslationMatrix(new Vertex(0d, 0d, 0d));
	...
	/**
	 * Returns an arbitrary vector that is parallel to the surface of this
	 * polygon.
	 */
	public Direction getSurfaceDirection() {
		...
		return new Direction(getEdge().getPoint(), getEdge().getNext()
				.getPoint());
	}
	...
	public Direction getNormal() {
		...
		Vertex L = edge.getNext().getNext().getPoint()
				.getTranslated(reverseRoot);
	...
	/**
	 * Intersect the plane with this polygon and return the list of points where
	 * they intersect. In the case of a non-convex polygon it is possible for
	 * more than two points to be returned. This uses (and is different from)
	 * Plane.intersect() because while the plane/polygon *planes* may intersect,
	 * they do not necessarily intersect within the bounds of the polygon.
	 */
			...
			for (int i = 0; i < vz.size(); i++) {
				...
				try {
					...
					if (intersection.intersectsAtPoint
						...
						if (!Vertex.listContainsSame(ret,
	...
	/**
	 * Create a new polygon that has the same points as this one but in the
	 * opposite order.
	 */
	...
	/**
	 * Returns a reasonable debugging string for this Polygon that includes the
	 * name and a space-delimited list of vertices.
	 */
	...
	/**
	 * Translate all my vertices by the given amount. Note: you might want to
	 * use the polyhedron transform method if this polygon is part of a
	 * polyhedron, because it will (1) affect other vertices, and (2) since
	 * polygons share vertices you might inadvertently translate the same vertex
	 * more than one time.
	 */
	...
	/**
	 * Find the longest line that joins the midpoints two nonadjacent edges.
	 * This works for Polygons that have 4 or more edges. (It will return null
	 * for a triangle.)
	 */
	public LineSegment findLongestMidline() {
		...
		for (int i = 0; i < segs.size(); i++) {
			for (int j = i + 2; j < i + (segs.size() - 1); j++) {
				k = j % segs.size();
				candidate = new LineSegment(segs.get(i).getMidpoint(), segs
						.get(k).getMidpoint());
	...
	/**
	 * Find the shortest line that joins the midpoints two nonadjacent edges.
	 * See findLongestMidline().
	 */
	...
	public void drawGL(SelectionModel sm) {
		...
		if (sel) {
			...
			// Debug.out("Polygon", "using color r: " + red + " g: " + green +
			// " b: " + blue);
	...
	void debugHalfEdges() {
		...
		do {
			counter++;
			pairString = cursor.getPair() == null ? "NO PAIR" : ""
					+ cursor.getPair().getID();
			nextString = cursor.getNext() == null ? "NO NEXT" : ""
					+ cursor.getNext().getID();
			faceString = cursor.getFace() == this ? "OK" : "Wrong Face!: "
					+ cursor.getFace();
			Debug.out("Polygon",
					"\t" + cursor.getID() + " " + cursor.getPoint() + " p: "
							+ pairString + " n: " + nextString + " f:"
							+ faceString);
			cursor = cursor.getNext();
			if (counter > 12) {
				Debug.out("Polygon",
						"\tIt is likely that you're in an infinite loop. Breaking.");
				break;
			}
		} while (cursor != edge);
	}
	...
	public void testIntersectionPoints() {
		Polygon triangle = new Polygon(new Vertex(0d, 1d, 0d), new Vertex(1d,
				0d, 0d), new Vertex(0d, 0d, 0d));
	...
	@Test
	public void testFindWhateverMidline() {
